...
public class FoundLostController extends BaseController{

	@Resource private IFoundLostService foundLostService;
	@RequestMapping(value="getFoundLostList",method=RequestMethod.POST)
	public Map<String, Object> getFoundLostList(HttpServletRequest request){
		...
		try {
			...
			//查询被过滤的数量
			List<Map<String, Object>> totalListFiltered = foundLostService.getFoundLosts(newMap);
			newMap.remove("selectCount");
			newMap.put("page", true);
			//查询被过滤的数据
			List<Map<String,Object>> pageList = foundLostService.getFoundLosts(newMap);
			//初始化分页返回数据
